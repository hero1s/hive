...
namespace ltimer {
	...
	luakit::lua_table open_ltimer(lua_State* L) {
		...
		luatimer.set_function("local_day", [](uint64_t ts) { return local_day(ts); });
		luatimer.set_function("month_days", [](uint64_t ts) { return month_days(ts); });
		...
		luatimer.set_function("day_begin_time", [](uint64_t ts) { return day_begin_time(ts); });
