...
namespace luakit {
    ...
    static thread_local luacodec* codec = nullptr;

    static codec_base* create_codec() {
        if (!codec) {
            codec = new luacodec();
    ...
    public:
        kit_state() {
            ...
            luakit.set_function("encode", [&](lua_State* L) { return encode(L, &thread_buff); });
            luakit.set_function("decode", [&](lua_State* L) { return decode(L, &thread_buff); });
            luakit.set_function("unserialize", [&](lua_State* L) {  return unserialize(L); });
            luakit.set_function("serialize", [&](lua_State* L) { return serialize(L, &thread_buff); });
        }
