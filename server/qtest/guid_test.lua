...
thread_mgr:fork(function()
    ...
    for i = 1, 10 do
        ...
        log_info("test guid:{}:{},serial:{}", i, guid, lcodec.guid_serial(guid))
        --log_info("test guid code:{}:{}", i, lcodec.guid_encode())
    end

    local guid = lcodec.guid_new(7, 54, 7)
    log_info("newguid: {}", guid)
    ...
    log_info("ssource-> group: {}, index: {},gtype:{}, time:{},serail:{}", group, index, gtype, datetime_ext.time_str(time), serail)

    thread_mgr:sleep(2000)
    guid = lcodec.guid_new(5, 34, 16)
    log_info("newguid: {}", guid)
    local group2, index2, gtype2, time2, serail = lcodec.guid_source(guid)
    log_info("nsource-> group: {}, index: {},gtype:{}, time:{},{}", group2, index2, gtype2, datetime_ext.time_str(time2), serail)
    ...
    for i = 1, 10 do
        for i = 1, 10000 do
            ...
            if guids[t] then
                logger.error("the guid is repeat:{}", t)
            end
            guids[t] = true
        end
    ...
    log_info("guid={} --> {} -- > {}", guid, guid_code, guid_num)
    local name     = "tencent"
    local name_num = lcodec.guid_decode(name)
    log_info("guid:{} -->{} --> {} ", name, name_num, lcodec.guid_encode(name_num))

    log_info("jumphash value: {},{},{}", lcodec.jumphash("", 3), lcodec.jumphash(-1, 3), lcodec.jumphash(0, 3))
