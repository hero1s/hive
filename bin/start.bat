...
start "router2 "%~dp0       ./hive.exe ./conf/router.conf      --index=2  --host_ip=127.0.0.1
start "monitor "%~dp0       ./hive.exe ./conf/monitor.conf     --index=1  --host_ip=%hostIp%
start "dbsvr1 "%~dp0        ./hive.exe ./conf/dbsvr.conf       --index=1
start "dbsvr2 "%~dp0        ./hive.exe ./conf/dbsvr.conf       --index=2
start "cachesvr1 "%~dp0     ./hive.exe ./conf/cachesvr.conf    --index=1
start "cachesvr2 "%~dp0     ./hive.exe ./conf/cachesvr.conf    --index=2
start "proxy1 "%~dp0        ./hive.exe ./conf/proxy.conf       --index=1
start "proxy2 "%~dp0        ./hive.exe ./conf/proxy.conf       --index=2
start "online1 "%~dp0       ./hive.exe ./conf/online.conf      --index=1
start "online2 "%~dp0       ./hive.exe ./conf/online.conf      --index=2
